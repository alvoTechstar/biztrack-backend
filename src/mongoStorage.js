...
export class MongoStorage {
  ...
  async updateUser(id, updateData) {
    try {
      ...
      delete dataToUpdate.password; // Don't update password directly
  ...
  async generateBusinessId() {
    const count = await this.Business.countDocuments({});
    return `BIZ-${(count + 1).toString().padStart(4, '0')}`;
  }
  ...
  async getBusinesses() {
    return await this.Business.find();
  }
  ...
  async getUserByBusinessId(businessId) {
    const user = await this.User.findOne({ associatedBusinessId: businessId });
    return user ?? undefined;
  }

  async getBusinessUsers(businessId) {
    return await this.User.find({ associatedBusinessId: businessId });
  }
